# EmoteBar - World of Warcraft Addon
...
1. Extract the EmoteBar folder to your World of Warcraft addons directory:
   `World of Warcraft\_retail_\Interface\AddOns\EmoteBar`

2. Launch World of Warcraft and enable the addon in the AddOns menu

3. The EmoteBar will appear in the center of your screen when you log in

## Commands

- `/emotebar` or `/eb` - Toggle EmoteBar visibility
- `/emotebar show` - Show the EmoteBar
- `/emotebar hide` - Hide the EmoteBar
- `/emotebar reset` - Reset EmoteBar position to center
- `/emotebar` (without arguments) - Show available commands
...
2. Save them in the `EmoteBar\Textures\` folder
3. Name your files to match the emotes (e.g., `wave.tga`, `hug.tga`, etc.)

### Required TGA Files:

Place these TGA files in the `EmoteBar\Textures\` folder:
...
1. Open `EmoteBar.lua` in a text editor
2. Find the `EmoteBar.emotes` table (around line 10)
...
Example:
```lua
{ name = "Custom", command = "/custom", icon = "Interface\\AddOns\\EmoteBar\\Textures\\custom.tga" }
```

## Positioning

- **Drag to Move**: Left-click and drag the EmoteBar to reposition it
- **Auto-Save**: Position is automatically saved when you stop dragging
- **Reset Position**: Use `/emotebar reset` to return to center screen

## Troubleshooting

### Icons Not Showing
1. Ensure TGA files are in the correct folder: `EmoteBar\Textures\`
2. Check file names match exactly (case-sensitive)
3. Verify TGA format is correct (uncompressed 24/32-bit)
4. Restart WoW after adding new texture files

### Addon Not Loading
1. Check that the folder is named exactly `EmoteBar`
2. Ensure all files are present: `EmoteBar.toc`, `EmoteBar.lua`, `EmoteBarFrame.xml`
...
```
EmoteBar/
├── EmoteBar.toc          # Addon metadata
├── EmoteBar.lua          # Main addon logic
├── EmoteBarFrame.xml     # UI frame definitions
├── README.md             # This file
